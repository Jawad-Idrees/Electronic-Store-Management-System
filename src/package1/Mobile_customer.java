...
public class Mobile_customer extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Mobile_customer frame = new Mobile_customer();
	...
	public Mobile_customer() {
		...
		ac=(ArrayList<Mobiles>)obj.readObject();}
        catch(Exception e) {
		...
		Object[] columns= {"Name", "Price", "Quantity","Rating","Code","Brand"};
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(j=0;j< ac.size();j++) {
						...
						if(ac.get(j).get_code()==product_code) {
							Mobiles s= new Mobiles();
							s=ac.get(j);
							Object[] search_code_display= new Object[] {s.get_name(),s.get_price(),s.get_quantity(),s.get_rating(),s.get_code(),s.get_brand()};
