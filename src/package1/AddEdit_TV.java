...
import java.io.*
;public class AddEdit_TV extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddEdit_TV frame = new AddEdit_TV();
	...
	try {
        ...
        else { ac=(ArrayList<TV>) obj.readObject();
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TV a= new TV(name_add.getText(), Integer.parseInt(price_add.getText()),Integer.parseInt(quantity_add.getText()),Double.parseDouble(rating_add.getText()),Product.generator(),Integer.parseInt(weight_add.getText() ));
				...
				ac.add(a);		
		            ...
		            try {
		            	...
		            	fout=new FileOutputStream("TV.bin");
					out=new ObjectOutputStream(fout);
					out.writeObject(ac);
					out.close();
					fout.close();
					JOptionPane.showMessageDialog(null, "Item added successfully");
					TV_section rf= new TV_section();
					rf.show();dispose();
				...
				if(code_edit.getText().trim().isEmpty()||name_edit.getText().trim().isEmpty()||price_edit.getText().trim().isEmpty()||rating_edit.getText().trim().isEmpty()||size_edit.getText().trim().isEmpty())
					...
					for(p=0;p<ac.size();p++) {
						if(!(ac.get(p).get_code()==codedit))continue;
							...
							ac.get(p).set_screen(Integer.parseInt(size_edit.getText()));
					...
					try {
						out= new ObjectOutputStream(new FileOutputStream("TV.bin"));
						out.writeObject(ac);
						out.close();
						...
						TV_section as= new TV_section(); as.show(); dispose();
		...
		JLabel lblNewLabel_1_1_1 = new JLabel("Screen size(inches)");
		lblNewLabel_1_1_1.setForeground(Color.WHITE);
		lblNewLabel_1_1_1.setBounds(77, 125, 131, 14);
		...
		JLabel lblNewLabel_4_1_1 = new JLabel("New screen size");
		lblNewLabel_4_1_1.setForeground(Color.WHITE);
		lblNewLabel_4_1_1.setBounds(126, 368, 103, 14);
