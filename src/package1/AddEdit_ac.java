...
import java.io.*
;public class AddEdit_ac extends JFrame {
	...
	private JTextField weight_edit;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddEdit_ac frame = new AddEdit_ac();
	...
	try {
        ...
        else { ac=(ArrayList<AC>) obj.readObject();
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				AC a= new AC(name_add.getText(), Integer.parseInt(price_add.getText()),Integer.parseInt(quantity_add.getText()),Double.parseDouble(rating_add.getText()),Product.generator(),Integer.parseInt(weight_add.getText()) );
				...
				ac.add(a);		
		            ...
		            try {
		            	...
		            	fout=new FileOutputStream("AC.bin");
					out=new ObjectOutputStream(fout);
					out.writeObject(ac);
					out.close();
					fout.close();
					JOptionPane.showMessageDialog(null, "Item added successfully");
					AC_section as= new AC_section(); as.show(); dispose();
		...
		btnEditItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(code_edit.getText().trim().isEmpty()||name_edit.getText().trim().isEmpty()||price_edit.getText().trim().isEmpty()||rating_edit.getText().trim().isEmpty()||weight_edit.getText().trim().isEmpty())
					...
					for(p=0;p<ac.size();p++) {
						if(!(ac.get(p).get_code()==codedit))continue;
							...
							ac.get(p).settons(Integer.parseInt(weight_edit.getText()));
					...
					try {
						out= new ObjectOutputStream(new FileOutputStream("AC.bin"));
						out.writeObject(ac);
						out.close();
		...
		JLabel lblNewLabel_1_1_1 = new JLabel("Weight(tons)");
		...
		JLabel lblNewLabel_4_1_1 = new JLabel("New weight");
		...
		weight_edit = new JTextField();
		weight_edit.setColumns(10);
		weight_edit.setBounds(239, 365, 151, 20);
		contentPane.add(weight_edit);
